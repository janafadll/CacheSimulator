...
unsigned int testAddresses[] = { 0x0000, 0x0004, 0x0010, 0x00FF, 0x0100, 0x1000, 0xFFFF, 0xABCD, 0x1234, 0xFFFFF };
...
int main()
{
    ...
    // Test Direct Mapped Cache with predefined addresses
    for (int clsIdx = 0; clsIdx < 4; clsIdx++) {
        int cls = cacheLineSizes[clsIdx];
        cout << "Direct Mapped Cache Simulator for Cache Line Size: " << cls << " bytes with predefined addresses" << endl;
        ...
        for (int i = 0; i < sizeof(testAddresses) / sizeof(testAddresses[0]); i++) {
            addr = testAddresses[i];         // Get predefined memory address
            r = cacheSimDM(addr, cls);       // Simulate cache access
            if (r == HIT) hit++;             // Increment hit counter if HIT
            cout << "0x" << setfill('0') << setw(8) << hex << addr << " (" << msg[r] << ")\n";
        }
        cout << "Hit ratio = " << (100.0 * hit / (sizeof(testAddresses) / sizeof(testAddresses[0]))) << "%" << endl; // Print hit ratio
        delete[] cacheDM; // Deallocate memory
    }

    // Test Fully Associative Cache with predefined addresses
    for (int clsIdx = 0; clsIdx < 4; clsIdx++) {
        int cls = cacheLineSizes[clsIdx];
        cout << "Fully Associative Cache Simulator for Cache Line Size: " << cls << " bytes with predefined addresses" << endl;
